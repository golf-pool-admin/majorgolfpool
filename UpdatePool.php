...
a:visited { color: #082744;} 
  ...
  <body>
    ...
    if ($wscore === '') $errors[] = 'Please enter a <b>winning score</b> pick.';
...
?>
  ...
  /*
    ...
    $subject = escapeshellarg('Your 2025 Masters Pool Entry!');
    $message = escapeshellarg(
        "Thank you, $name, for entering the pool!\n\n" .
        "Team Name: $teamname\nWinning Score: $wscore\n\n" .
        implode("\n", array_values($holes)) . "\n\n" .
        "Follow all the action at: http://www.majorgolfpools.ca"
    );
    ...
    $emailCommand = "echo $message | mail -s $subject -a 'From: info@majorgolfpools.ca' -a 'Reply-To: info@majorgolfpools.ca' -a 'Bcc: golf.pool.admin@gmail.com' $to";

    // Run the command in the background
    exec("$emailCommand > /dev/null 2>&1 &");

    echo "<p>We have received your entry! A confirmation email will be sent shortly.<br><br></p>";
    //
    ...
    // Output the confirmation message immediately
    echo "<p>We have received your entry! A confirmation email will be sent shortly.<br><br></p>";
    // Flush the output buffer to send the response to the browser
    ...
    // Send the email after responding to the user
    if (!mail($to, $subject, $message, $headers)) {
        error_log("Failed to send email to $email");
    }
*/
    // After processing the form, store email data in a temporary file
$emailData = [
    'to' => $email,
    'name' => $name,
    'teamname' => $teamname,
...
$filename = '/tmp/email_queue_' . time() . '_' . rand(1000, 9999) . '.json';
...
// Trigger the background email processor without waiting for it
// Using relative path since both files are in the same directory
exec("php $currentDir/send_emails.php > /dev/null 2>&1 &");

// Immediately show success message to user
echo "<p>We have received your entry! A confirmation email will be sent shortly.<br><br></p>";
